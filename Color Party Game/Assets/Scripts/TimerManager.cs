...
public class TimerManager : PunRaiseEvents
{
    ...
    public override void OnEvent(EventData photonEvent)
    {
        ...
        {
            case (byte)TimeRaiseEventsCode.INITIAL_COUNTDOWN:
                object[] data = (object[])photonEvent.CustomData;
                StartCoroutine(InitiateCountdown());
                break;

            case (byte)TimeRaiseEventsCode.TIMER:
                StartCoroutine(Timer());
                break;

            case (byte)TimeRaiseEventsCode.TIME_UP:
                StartCoroutine(TimeOver());
                break;
        }
    }
    public override void SetRaiseEvent()
    {
        ...
        switch (GameManager.Instance.GameState)
        {
            case GameManager.GameStates.INITIAL:
                PhotonNetwork.RaiseEvent((byte)TimeRaiseEventsCode.INITIAL_COUNTDOWN, data, raiseEventOptions, sendOption);
                break;

            case GameManager.GameStates.PLAYING:
                PhotonNetwork.RaiseEvent((byte)TimeRaiseEventsCode.TIMER, data, raiseEventOptions, sendOption);
                break;

            case GameManager.GameStates.GAME_OVER:
                PhotonNetwork.RaiseEvent((byte)TimeRaiseEventsCode.TIME_UP, data, raiseEventOptions, sendOption);
                break;
        }
    }
